    ...
    <div className="aurelia-site">
      ...
      <main>
        <section className="store-hero">
          ...
          <div className="store-hero__content"><p className="eyebrow">{hero?.eyebrow || "CRAFTED FOR MOMENTS"}</p><h1>{hero?.title || <>Scents that leave a <em>lasting impression.</em></>}</h1><p>{hero?.body || "Modern perfume, thoughtfully crafted with the finest ingredients from around the world."}</p><div className="hero-actions"><button className="solid-link" onClick={() => hero?.ctaHref ? window.location.assign(hero.ctaHref) : document.getElementById("best-sellers")?.scrollIntoView({ behavior: "smooth" })}>{hero?.ctaLabel || "Explore collections"} <ArrowDown size={17} /></button><button className="text-link text-link--ink" onClick={() => document.getElementById("categories")?.scrollIntoView({ behavior: "smooth" })}>Discover your scent <ArrowRight size={17} /></button></div></div>
        ...
        <section className="assurance-row" aria-label="Store benefits"><p><Sparkles size={15} />Fine ingredients</p><p>Long-lasting scents</p><p>Elegant packaging</p><p>Made with passion</p></section>

        <section className="category-section" id="categories"><SectionHeading overline="SHOP BY COLLECTION" title={<>Find your signature <em>scent.</em></>} copy="Thoughtfully crafted fragrances for every mood, moment, and ritual." /><div className="category-grid">{liveCategories.map((category, index) => <button key={category.title} className={`category-card category-card--${category.position} ${index === 2 ? "category-card--feature" : ""}`} onClick={() => toast(`Exploring the ${category.title} collection.`)}><img src={category.image} alt="" /><span className="category-card__wash" /><span className="category-card__content"><small>0{index + 1}</small><strong>{category.title}</strong><em>{category.note}</em><i>Explore <ArrowRight size={15} /></i></span></button>)}</div></section>

        {bestSellerSlot?.enabled !== false && <section className="product-section" id="best-sellers"><SectionHeading overline={bestSellerSlot?.eyebrow || "FEATURED FRAGRANCES"} title={bestSellerSlot?.title || <>Find your new <em>signature.</em></>} copy={bestSellerSlot?.body || "Our most loved scents, selected for their lasting impression."} action="View all fragrances" /><div className="commerce-grid commerce-grid--four">{liveBestSellers.map((product) => <ProductCard key={product.id} product={product} />)}</div></section>}
