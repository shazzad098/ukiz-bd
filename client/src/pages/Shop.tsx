...
import { fragranceFamilies, storefrontNav, type FragranceFamily, type ProductCategory } from "@/data/catalog";
...
export default function Shop() {
  ...
  const [category, setCategory] = useState<ProductCategory | "All">(() => initialCategory && initialCategory !== "Shop" ? initialCategory as ProductCategory : "All");
  ...
  const categoryOptions = managedCategories.data === undefined ? storefrontNav.filter((item) => item !== "Shop") : managedCategories.data.map((category: any) => category.name);

  const brands = useMemo(() => ["All", ...Array.from(new Set(products.map((product) => product.brand)))], [products]);
  const suggestions = useMemo(() => query.trim().length < 2 ? [] : products.filter((product) => `${product.name} ${product.brand} ${product.category} ${product.family}`.toLowerCase().includes(query.toLowerCase())).slice(0, 5), [products, query]);
  ...
  }, [brand, category, families, gender, inStockOnly, maxPrice, products, query, ratingFilter, sort]);
