...
const adminVariantInput = z.object({ size: z.string().trim().min(1).max(32), sku: z.string().trim().min(1).max(128), price: z.number().nonnegative().max(1_000_000), originalPrice: z.number().nonnegative().max(1_000_000).nullable().optional(), stock: z.number().int().min(0).max(1_000_000), published: z.boolean().optional() });
